...
VM_MONGODB_CMD="nohup /usr/bin/mongod --config $HOME/group-project-master/env/mongodb_vm_$ENV.conf &"
...
GRAPHIC_SERVER_STARTSTOP_SCRIPT="./$HOME/group-project-master/bin/startstop_graphic.sh -a $ACTION -e $ENV -s $STUBS"
...
if [ ! "`hostname -i`" == "146.169.44.217" ] ; then 
	echo "script must be run from VM machine"
	exit 2
fi
...
case "$ACTION" in
  start)
	...
	ps -ef | grep httpserver.js | grep -v grep |  awk '{print $2}' > /tmp/node_vm_$ENV.pid
	...
	if $DISTRIBUTED ; then 
		`${SSH_GRAPHIC} ${GRAPHIC_SERVER_STARTSTOP_SCRIPT}`
